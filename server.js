...
const updateEmpRole = async () => {
  try {
    ...
    const empSQL =
      'SELECT employee.id, employee.firstName, employee.lastName, role.title FROM employee LEFT JOIN role ON employee.role_id = role.id;'
    const [result] = await connection.query(empSQL)
    const employees = result.map(({ firstName, lastName, title, id }) => ({
...
const updateEmpManager = async () => {
  try {
    ...
    const empSQL =
      'SELECT employee.id, employee.firstName, employee.lastName, role.title FROM employee INNER JOIN role ON employee.role_id = role.id;'
    const [result] = await connection.query(empSQL)
    ...
    inquirer
      ...
      .then((empChoice) => {
        ;(async () => {
          ...
          const managers = result.map(({ firstName, lastName, title, id }) => ({
            name: `${firstName} ${lastName} ${title}`,
            value: id,
          }))
...
const employeeByDept = async () => {
  ...
  try {
    ...
    const depts = result.map(({ Name, Id }) => ({
      name: Name,
      value: `${Id}, ${Name}`,
    }))
      ...
      .then((deptChoice) => {
        ...
        ;(async () => {
          try {
            ...
            const [empByDept] = await connection.query(
              empSQL,
              Number(deptFields[0])
            )
...
const deleteEmployee = async () => {
  ...
  try {
    ...
    const empSQL =
      'SELECT employee.id, employee.firstName, employee.lastName, role.title FROM employee INNER JOIN role ON employee.role_id = role.id;'
    const [result] = await connection.query(empSQL)
    const employees = result.map(({ firstName, lastName, id }) => ({
    ...
    inquirer
      ...
      .then((deptChoice) => {
        ...
        ;(async () => {
          ...
          const delDept = await connection.query(
            delDeptSQL,
            Number(deptFields[0])
          )
    ...
    inquirer
      ...
      .then((roleChoice) => {
        ...
        ;(async () => {
          ...
          const delRole = await connection.query(
            delRoleSQL,
            Number(roleFields[0])
          )
...
const viewEmpByManager = async () => {
  ...
  try {
    ...
    const managers = result.map(({ firstName, lastName, id }) => ({
      name: `${firstName} ${lastName}`,
      value: `${id}, ${firstName}, ${lastName}`,
    }))
      ...
      .then((managerChoice) => {
        ...
        ;(async () => {
          try {
            ...
            const [empByManager] = await connection.query(
              empSQL,
              Number(managerFields[0])
            )
            console.log(
